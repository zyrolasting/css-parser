...
In Racket, each token type is a transparent @racket[struct] that
tracks the line and column at which the token begins in CSS code. The
tokenizer derives source location data from
...
@sl{5.3}, and the struct definitions representing parsed CSS3 code.
